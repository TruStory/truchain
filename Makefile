...
init:
	...
	bin/truchaind add-genesis-account $(shell bin/truchaincli keys show registrar -a --home ~/.octopus) 1000000000tru
	sed -i -e 's/bond_denom.*/bond_denom": "tru"/' ~/.truchaind/config/genesis.json
	...
	bin/truchaind gentx --name=registrar --amount 100000000tru --home-client ~/.octopus
